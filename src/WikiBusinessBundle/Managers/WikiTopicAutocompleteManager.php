...
class WikiTopicAutocompleteManager extends AbstractAutocompleteManager
{
    ...
    public function getAutoComplete($term, $attribute, $formData)
    {
        ...
        if (array_key_exists("parent_page_id", $formData)) {

            $etWikiPage = $this->entityManager->getEntityTypeByCode("wiki_page");

            /** @var WikiPageEntity $parentPage */
            $parentPage = $this->entityManager->getEntityByEntityTypeAndId($etWikiPage, $formData["parent_page_id"]);
            if (!empty($parentPage)) {
                $compositeFilter->addFilter(new SearchFilter("id", "eq", $parentPage->getParentTopicId()));
            }
        }
