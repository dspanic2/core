...
class ProductAutocompleteManager extends AbstractAutocompleteManager
{
    ...
    {
        $productEntityType = $this->entityManager->getEntityTypeByCode("product");
    ...
    protected function buildCompositeFilters($term, $formData, $productEntityType)
    {
        ...
        if (!empty($term)) {
            ...
            $compositeFilter->addFilter(new SearchFilter("code", "bw", $term));
    ...
    protected function buildSortFilters()
    {
        ...
        $sortFilters->addSortFilter(new SortFilter("code", "asc"));
        return $sortFilters;
    }
    ...
    public function renderTemplate($data, $template, $attribute)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $key => $d) {
                $ret[$key]["id"] = $d->getId();
                $ret[$key]["html"] = $this->twig->render("CrmBusinessBundle:AutocompleteTemplates:product.html.twig", array('field_data' => $d, 'attribute' => $attributeCode));
            }
        }
    ...
    {
        $ret["id"] = $item->getId();
        $ret["lookup_value"] = $this->twig->render("CrmBusinessBundle:AutocompleteTemplates:product.html.twig", array('field_data' => $item, 'attribute' => $attributeCode));
    ...
    {
        return "CrmBusinessBundle:AutocompleteTemplates:product.html.twig";
    }
    ...
    {
        return "CrmBusinessBundle:AutocompleteTemplates:product_added_entities.html.twig";
    }
}
