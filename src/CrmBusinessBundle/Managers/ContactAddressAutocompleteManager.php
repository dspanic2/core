...
class ContactAddressAutocompleteManager extends AbstractAutocompleteManager
{
    ...
    {
        if (!isset($formData["contact_id"]) || empty($formData["contact_id"])) {
            return array();
        }
        ...
        $contact = $this->accountManager->getContactById($formData["contact_id"]);
        ...
        $compositeFilter->addFilter(new SearchFilter("account", "eq", $contact->getAccountId()));
