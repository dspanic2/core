...
class DiscountCartRulesController extends AbstractController
{
    ...
    /**
     * @Route("/discount_cart_rule/save", name="discount_cart_rule_save_form")
     * @Method("POST")
     */
    public function saveAction(Request $request)
    {
        ...
        $type = "discount_cart_rule";

        $this->initializeForm($type);

        if(empty($_POST["rules"]) || empty(json_decode($_POST["rules"]))){
            return new JsonResponse(array('error' => true, 'title' => $this->translator->trans('Error occured'), 'message' => $this->translator->trans('Discount cart rules cannot be empty')));
        }
