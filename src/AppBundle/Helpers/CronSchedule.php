...
class CronSchedule
{
	...
	public function setVariables($schedule){
	    $this->_minutes = $schedule->_minutes;
	    $this->_hours = $schedule->_hours;
	    $this->_daysOfMonth = $schedule->_daysOfMonth;
	    $this->_months = $schedule->_months;
	    $this->_daysOfWeek = $schedule->_daysOfWeek;
	    $this->_years = $schedule->_years;
	    $this->_cronMinutes = $schedule->_cronMinutes;
	    $this->_cronHours = $schedule->_cronHours;
	    $this->_cronDaysOfMonth = $schedule->_cronDaysOfMonth;
	    $this->_cronMonths = $schedule->_cronMonths;
	    $this->_cronDaysOfWeek = $schedule->_cronDaysOfWeek;
	    $this->_cronYears = $schedule->_cronYears;
	...
	final public function match($time = FALSE)
	{
		...
		// Years
		if(!array_key_exists($arrDT[4], $this->_years) && !empty($this->_years)) return FALSE;
		// Day of week
	...
	final public function matchByArray($time = FALSE, $schedule)
	{
		...
		// Years
		if(!array_key_exists($arrDT[4], $schedule->_years) && !empty($schedule->_years)) return FALSE;
		// Day of week
		if(!array_key_exists(date('w', strtotime($arrDT[4].'-'.$arrDT[3].'-'.$arrDT[2])), $schedule->_daysOfWeek)) return FALSE;
		// Month
		if(!array_key_exists($arrDT[3], $schedule->_months)) return FALSE;
		// Day of month
		if(!array_key_exists($arrDT[2], $schedule->_daysOfMonth)) return FALSE;
		// Hours
		if(!array_key_exists($arrDT[1], $schedule->_hours)) return FALSE;
		// Minutes
		if(!array_key_exists($arrDT[0], $schedule->_minutes)) return FALSE;
	...
	final public function next($time = FALSE)
	{
		...
		while(1)
		{
			...
			if(!array_key_exists($arrDT[4], $this->_years) && !empty($this->_years))
			{
	...
	final public function getIntervalInSeconds($time = false, $schedule): int
    {
        $this->setVariables($schedule);

        $diffInSeconds = $this->nextAsTime($time) - $this->previousAsTime($time);
        if ($this->match($time)) {
    ...
    public function getPreviousDateTime($time = false, $schedule): int
    {
        $this->setVariables($schedule);

        return $this->previousAsTime($time);
    }
}