...
function MenuEditor(idSelector, options) {
    ...
    function editItem(item) {
        ...
        $.each(data, function (p, v) {
            ...
            console.log($form.find("[name=" + p + "]").val());
        });
        ...
        if (jQuery('#menu_item_type').length > 0) {
            jQuery('#menu_item_type').trigger('change');
        }
    }
    ...
    this.updateSmenu = function () {
        var $cEl = this.getCurrentItem();
        if ($cEl === 0) {
            return;
        }

        var type = $form.find('#menu_item_type').val();
        if (type == 2 && !$form.find('#page').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_page
            });
            return false;
        } else if (type == 3 && !$form.find('#product_group').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_product_group
            });
            return false;
        } else if (type == 4 && !$form.find('#url').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_url
            });
            return false;
        } else if (type == 5 && !$form.find('#blog_category').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_page
            });
            return false;
        } else if (type == 6 && !$form.find('#brand').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_page
            });
            return false;
        } else if (type == 7 && !$form.find('#warehouse').val()) {
            jQuery.growl.error({
                title: translations.error_message,
                message: translations.please_select_page
            });
    ...
    this.addSmenu = function () {
        var data = {};

        if (!jQuery.trim($form.find('[name="text"]').val())) {
            return false;
        }
